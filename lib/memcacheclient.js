...
function getter(verb) {
  return function(keys, cb) {
    this.run(keys, function(socket) {
      ...
      return socket.enqueue(GetCommand(verb, keys, cb))
    })
  }
}
...
function setter(verb) {
  return function(key, value, opts, cb) {
    this.run(key, function(socket) {
      ...
      return socket.enqueue(SetCommand(verb, key, value, opts, cb))
    })
  }
}
...
MemcacheClient.prototype = {
  ...
  del: function(key, opts, cb) {
    this.run(key, function(socket) {
      ...
      return socket.enqueue(DeleteCommand('delete', key, opts, cb))
    })
  }
}
